...
- in-memory backend: demo / test 用
- object backend: 正本の永続実行用
- SQLite backend: optional adapter

object backend は JSON object を正本にし、session・memory・embedding・graph・checkpoint を同じ root 配下に保存する。
...
SQLite adapter は session・memory・graph・checkpoint を同じ DB に保存する。主要テーブルは次である。
...
- graph traversal 用に relation predicate index を持つ
...
model は `ModelRunner` trait 越しに使う。現在の実装は `RuleBasedModelRunner` であり、production-grade runtime の baseline として deterministic behavior を優先している。実 LLM backend は後で差し替えられる。
...
現在の baseline 実装は `SimpleDistiller` であり、deterministic に graph fragment を構築する。
