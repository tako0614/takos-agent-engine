...
`build_execution_graph_preset` は `Default` / `Planner` / `Subgoal` の preset を返す。
`Planner` は tool / distillation なしで model 出力を `plan` に反映し、`Subgoal` は bounded tool loop と応答永続化だけを行う軽量 graph である。
...
`state_json` には `ExecutionState` 全体を保存する。現在の public `resume_loop`
...
MVP の ANN-like 実装として opt-in の `AnnVectorIndex` を公開する。これは
deterministic random-projection / LSH bucket で candidate を絞り、最終 ordering
は exact cosine score 降順・ID 昇順で rerank する。外部 ANN dependency は持たず、
小規模 index では default candidate 数により exact search と同じ候補集合になる。
...
checkpoint の保存・読込・削除を担当する。graph node の前で `Running` checkpoint
...
として checkpoint に残す。これらは状態を失わないための terminal checkpoint
であり、`resume_loop` の通常再開対象ではない。
...
model は `ModelRunner` trait 越しに使う。crate 本体は default では concrete model runner を公開せず、example と test support に deterministic toy 実装を置く。短期 MVP として `openai-chat` feature で OpenAI-compatible chat/completions backend を公開する。
...
crate 内 API として `FederatedMemoryTools` を提供する。これは明示的に渡された
`MemorySource { source_id, repository, vector_index, graph_repository? }`
群に対する read-only retrieval primitive であり、source discovery は行わない。
`semantic_search` は query embedding を一度だけ作り、各 source の raw /
abstract vector index を検索して、`source_id` 付き hit に merge する。
merge order は score 降順、`source_id` 昇順、node id 昇順の deterministic
ranking であり、`threshold` と bounded `top_k` を適用する。raw / abstract
は `MemorySearchTarget` で選択できる。
`graph_search` は `graph_repository` を持つ source だけを対象に predicate
filter 付き traversal を行い、`source_id` 付き hit として返す。
`timeline_search` は各 source の `NodeRepository` から global / session-scoped
raw timeline を読み、timestamp 降順、`source_id` 昇順、raw id 昇順で merge する。
...
- feature-gated OpenAI-compatible chat/completions backend
- planner / subgoal graph preset MVP
- opt-in ANN-like vector index MVP
- multi-agent memory federation crate API MVP

crate 外の統合課題:

- 分散 scheduler
- multi-agent memory federation の write / replication / auth / source
  discovery
