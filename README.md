...
activation しながら checkpoint 可能な graph runtime で長期継続実行する。 Takos monorepo には runtime integration 用の
copy が `takos/packages/rust-agent-engine/` に置かれ、 `takos/apps/rust-agent/` が container wrapper
としてこれを利用する。 agent runtime の境界は [Agent Runtime](docs/agent-runtime.md) を参照。
...
GraphRunner は side effect 境界の前後で checkpoint を取る。process restart、timeout、cancellation、明示的な pause
のいずれで止まっても、`resume_loop` で直前の node から再開できる。tool 実行の途中で落ちても operation_key による
idempotent persistence が二重書き込みを防ぐ。
...
を含む。cancellation / timeout は `LoopStatus::Cancelled` / `LoopStatus::TimedOut` として checkpoint
に残り、状態が失われない。
