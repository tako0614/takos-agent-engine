...
::: tip Status
このページは current implementation の agent runtime 境界を説明します。Takos は control plane 全体を Rust
...
`skill_list` / `skill_get` / `skill_context` / `skill_catalog` / `skill_describe` の local intercept は
`takos/apps/rust-agent/` 側の wrapper が担い、managed skill と custom skill の合成結果を返します。
...
`skill_*` 系は `takos-agent-engine` の公開 tool surface ではなく、`takos/apps/rust-agent/` の wrapper が local intercept
します。
...
- monorepo integration copy: `takos/packages/rust-agent-engine/` は Takos monorepo 側の runtime integration 用 copy
  で、container wrapper から参照される
- container app: `takos/apps/rust-agent/` は bootstrap / control RPC / prompt / skill wiring を担う wrapper
- control RPC contract: Takos control plane 側の container-hosts / executor 間 RPC 定義

Takos-agent の Rust 実装は、engine core を standalone `takos-agent-engine/` に置き、Takos monorepo 側では runtime
integration 用 copy と container wrapper に分けて扱う。
